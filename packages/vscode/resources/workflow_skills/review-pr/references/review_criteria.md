...
*Same priority as Design — named requirements from the assigned task or tech design.*

Verify **implementation** requirements only: code, behavior, APIs, UI contract, and
tests named as deliverables in the assigned task Description / Subtasks or in
`technical-design.md`.

**Out of scope for review** (do not file findings or REQUEST_CHANGES for these):
process / workflow bullets such as commit, push, open or merge a PR, “mark done”,
local dependency install, or “run the full suite before done” when the review
signal is this PR’s check-runs (or none). By review time the PR already exists;
those steps belong to the impl agent / wrapper.

| Check | Blocker? |
|---|---|
| Named requirement in the assigned task Description / Subtasks or `technical-design.md` not followed (symbol rename, icon, route, copy, API field name, component, label, state text, etc.) | 🟡 |
| Same mismatch when the item is an acceptance criterion or must-have deliverable (including a partially implemented in-scope subtask) | 🔴 |

**Hard rule:** if a finding cites a concrete approved value from the task or tech
design, severity is **never** 🟢. Do not downgrade as “cosmetic”, “still works”,
or “still navigable.”
...
- [ ] Every **in-scope** named requirement from the assigned task Description / Subtasks is verified in the diff (Spec fidelity) — process bullets (commit, push, open PR, etc.) skipped and not used to block
- [ ] Any in-scope named mismatch recorded as ≥ 🟡 — never 🟢 / “cosmetic”
- [ ] All in-scope subtasks from `tasks.md` are implemented (Correctness)
...
- [ ] CI for **this** PR: no check-runs → OK; present check-runs all terminal and not failed/cancelled/pending; fetch failure → treat as CI unknown and block (Correctness). Do not require checks that only run on `main` or other bases
...
- [ ] Implementation matches the architecture **and** named details in `technical-design.md` (Design / Spec fidelity)
...
- [ ] Cannot APPROVE if any in-scope checklist item above failed
